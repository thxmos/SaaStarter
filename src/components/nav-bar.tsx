		...
		<nav className='flex items-center justify-between w-screentext-lg absolute w-screen z-20'>
				...
				{user && user.avatar && (
					<Link href='/dashboard'>
						<Avatar>
							...
							<AvatarFallback className='bg-red-500 text-white text-xs'>{getInitials(user.name!)}</AvatarFallback>
						</Avatar>
					</Link>
				)}
			...
			>
				<div className='flex flex-col gap-4 p-12'>
					<Link href='/'>
						<p className='py-4 px-4 hover:bg-card transition-all duration-300 rounded-md'>Home</p>
					</Link>
					<Link href='/dashboard'>
						<p className='py-4 px-4 hover:bg-card transition-all duration-300 rounded-md'>Dashboard</p>
					</Link>
					{user && (
